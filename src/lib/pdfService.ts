  ...
  private static async generateInvoicePDFWithPDFKit(invoice: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.fontSize(10)
        doc.text('T&V Servicios y Complementos S.L.', 400, 50, { align: 'right' })
        doc.text('CIF B-86715893', 400, 65, { align: 'right' })
        doc.text('Registro RI-AZE con el número 17208', 400, 80, { align: 'right' })

        // Título de factura
        doc.moveDown(4)
        doc.fontSize(20).font('Helvetica-Bold')
        doc.text(`FACTURA ${invoice.prefix}${invoice.invoice_number}${invoice.suffix}`, 50, 150)

        // Datos del cliente
        doc.fontSize(12).font('Helvetica-Bold')
        doc.text('Facturar a:', 50, 200)
        ...
        doc.text('Precio (sin IVA)', 370, tableTop)
        doc.text('IVA', 480, tableTop)
        ...
        yPosition += 20
        doc.fontSize(10).font('Helvetica')
        doc.text('Base imponible', 370, yPosition)
        doc.text(formatPrice(baseImponible), 480, yPosition)
        
        yPosition += 20
        doc.text('IVA 21%', 370, yPosition)
        doc.text(formatPrice(iva), 480, yPosition)
        
        yPosition += 20
        doc.fontSize(12).font('Helvetica-Bold')
        doc.text('Total', 370, yPosition)
        doc.text(formatPrice(totalCents), 480, yPosition)
