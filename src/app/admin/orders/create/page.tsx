...
export default function AdminOrderCreate() {
  ...
  const [orderData, setOrderData] = useState<NewOrderData>({
    ...
    items: [{ product_title: '', qty: 1, price_cents: 0 }]
  })
  ...
  const addOrderItem = () => {
    setOrderData({
      ...orderData,
      items: [...orderData.items, { product_title: '', qty: 1, price_cents: 0 }]
    })
  }
  ...
  const removeOrderItem = (index: number) => {
    ...
    setOrderData({
      ...orderData,
      items: newItems.length > 0 ? newItems : [{ product_title: '', qty: 1, price_cents: 0 }]
    })
  }
  ...
  const copyBillingToShipping = () => {
    setOrderData({
      ...orderData,
      shipping_address: { ...orderData.billing_address }
    })
  }
  ...
  const handleSave = async () => {
    try {
      ...
      if (!orderData.billing_address.address_line1 || !orderData.billing_address.city) {
        alert('Debe completar la dirección de facturación')
        return
      }
      
      if (!orderData.shipping_address.address_line1 || !orderData.shipping_address.city) {
        alert('Debe completar la dirección de envío')
        return
      }
      
      if (orderData.items.some(item => !item.product_title || item.qty <= 0 || item.price_cents <= 0)) {
        alert('Todos los artículos deben tener nombre, cantidad y precio válidos')
        return
      }
